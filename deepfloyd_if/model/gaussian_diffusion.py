...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        return {'sample': sample, 'pred_xstart': out['pred_xstart'], 'mean': out['mean'], 'pred_eps': out['pred_eps']}
    ...
    def p_sample_loop(
        ...
        for step_idx, sample in enumerate(self.p_sample_loop_progressive(
            ...
            final = sample
            mid_img.append(sample['sample'])
            x0_pred.append(sample['pred_xstart'])
            mean_img.append(sample['mean'])
            step_num.append(step_idx)
            # x0_pred_random.append(sample['x0_pred_random'])
            pred_eps.append(sample['pred_eps'])

        return final['sample'], mid_img, x0_pred, step_num, mean_img, pred_eps
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with torch.no_grad():
                ...
                # rand_eps = torch.randn_like(img).to(img)
                # out['x0_pred_random'] = self._predict_xstart_from_eps(img, t, rand_eps)
                
            
                yield out
                if i == (99 - 100):
                    first_img_con = out['sample'][:1].repeat(9, 1, 1, 1)
                    # first_img_uncon = out['sample'][9:10].repeat(9, 1, 1, 1)
                    # first_img_uncon = out['sample'][9:]
                    first_img_uncon = torch.randn_like(out['sample'][9:]).to(out['sample'][9:])
                    img = torch.cat((first_img_con, first_img_uncon), 0)
