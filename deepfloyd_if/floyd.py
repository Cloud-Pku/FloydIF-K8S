...
from .modules.utils import load_model_weights, predict_proba, clip_process_generations
...
class FloydDM:
    ...
    available_models = ['IF-I-M-v1.0', 'IF-I-XL-v1.0', 'IF-II-L-v1.0']
    ...
    def __init__(self, dir_or_name, device, pil_img_size=64, model_kwargs=None, cache_dir=None, hf_token=None):
    ...
    def embeddings_to_image(
        ...
        **kwargs,
        ...
        if support_noise is None:
            noise = torch.randn(
                (1, 3, image_h, image_w), device=self.device, dtype=self.model.dtype)
            ...
            print(support_noise.shape, (1, 3, image_h, image_w))
        ...
        noise_fig = self.to_images(noise[:9], disable_watermark=True)

        self.savefig(noise_fig, fname='noise')
        if inpainting_mask is not None:
            inpainting_mask = inpainting_mask.to(device=self.device, dtype=torch.long)

        if sample_loop == 'ddpm':
            with torch.no_grad():
                sample, mid_img, x0_pred, step_num, mean_img, pred_eps = diffusion.p_sample_loop(
                    model_fn,
        ...
        print("validating...")
        # for i in tqdm(step_num):
        #     mid_img[i] = self.__validate_generations(mid_img[i])
        #     x0_pred[i] = self.__validate_generations(x0_pred[i])
        sample = self.__validate_generations(sample)
        self._clear_cache()

        return sample, metadata, mid_img, x0_pred, step_num, mean_img, pred_eps

    def load_conf(self, dir_or_name, filename='config.yml'):
        path = self._get_path_or_download_file_from_hf(dir_or_name, filename)
        # path = '/mnt/nfs/chenyun/IF/weights/IF_/' + path
    ...
    def _get_path_or_download_file_from_hf(self, dir_or_name, filename):
        if dir_or_name not in self.available_models:
            ...
            return os.path.join(self.cache_dir, dir_or_name, filename)
    ...
    def savefig(self, pil_images, nrow=None, size=10, fname='fig'):
        if nrow is None:
        ...
        # fix.savefig('/mnt/nfs/chenyun/IF/deepfloyd_if/fix.png')
        plt.savefig('/mnt/nfs/chenyun/IF/img/{}.png'.format(fname))
        
         
    def show(self, pil_images, nrow=None, size=10, fname='fig'):
        if nrow is None:
            nrow = round(len(pil_images)**0.5)
        ...
        # fix.savefig('/mnt/nfs/chenyun/IF/deepfloyd_if/fix.png')
        plt.savefig('/mnt/nfs/chenyun/IF/{}.png'.format(fname))
        print('plot done')
